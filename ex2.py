...
def saveplots(plotpath,Af,target):
...
def run():
	...
	trainer=learning.Trainer(learner,X,Y,validationbatchsize=samples_val)
	...
	while time.perf_counter()<timebound:
		if stopwatch.elapsed()<checkpoint_interval:
			...
			#trainer.validation()
			trainer.save()
			saveplots(plotpath,learner[0],target)
...
if __name__=='__main__':
	...
	slate.addspace(2)
	slate.addtext('validation loss')
	slate.addbar(lambda tk:tk.get('validation loss'),emptystyle='.')
