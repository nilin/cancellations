...
def plot(key,folder,ac_name):
	...
	n_squares={n:s[:min(100000,s.size)] for n,s in n_squares.items()}

	plt.plot(n_squares.keys(),[jnp.average(s) for _,s in n_squares.items()],color='b')
...
if __name__=='__main__':
	...
	plot(key,'seed=123/','ReLU')
	plot_gamma(key,'seed=0/','ReLU')
	plot_packing()
